...
public class DataScopeAspect
{
    ...
    public static void dataScopeFilter(JoinPoint joinPoint, User user, String deptAlias, String userAlias)
    {
        ...
        for (Role role : user.getRoles())
        {
            ...
            System.out.println("切面数据==="+dataScope);
            //全部数据权限
        ...
        if (StringUtil.isNotBlank(sqlString.toString()))
        {
        	if(joinPoint.getArgs()[0] instanceof BaseEntity){
        		System.out.println("是实体类");
        		 BaseEntity baseEntity = (BaseEntity) joinPoint.getArgs()[0];
                 baseEntity.getParams().put(DATA_SCOPE, " AND (" + sqlString.substring(4) + ")");
        	}else {
        		BaseEntity baseEntity = new BaseEntity();
        		Map<String, Object> objetMap = (Map<String, Object>) joinPoint.getArgs()[0];
        		objetMap.put(DATA_SCOPE, " AND (" + sqlString.substring(4) + ")");
        		baseEntity.setParams(objetMap);
        	}
        }
    }
