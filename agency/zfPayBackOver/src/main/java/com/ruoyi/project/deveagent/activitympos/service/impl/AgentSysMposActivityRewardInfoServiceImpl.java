...
import com.ruoyi.project.deveagent.activitympos.mapper.AgentSysMposActivityRewardInfoMapper;
 ...
 */
@Service
public class AgentSysMposActivityRewardInfoServiceImpl implements AgentSysMposActivityRewardInfoService {
	...
	@Autowired
	private AgentSysMposActivityRewardInfoMapper agentSysMposActivityRewardInfoMapper;
	 ...
	 */
	@Override
	public List<Map<String, Object>> getAgentSysMposActivityRewardInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoList(params);
	}
	 ...
	 */
	@Override
	public List<AgentSysMposActivityRewardInfo> selectAgentSysMposActivityRewardInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentSysMposActivityRewardInfoMapper.selectAgentSysMposActivityRewardInfoList(params);
	}
	 ...
	 */
	@Override
	public R addAgentSysMposActivityRewardInfos(Map<String, Object> params) {
		try {
			...
			//根据活动id查询活动详情
			Map<String, Object> activityInfo = agentSysMposActivityInfoMapper.getAgentSysMposActivityInfoById(params.get("activity_id").toString());
	        ...
	        for(int i=0;i<pos_nums.length;i++) {
	        	...
	        	//依次新增每一个活动奖励类型
	        	R result = SpringUtils.getAopProxy(this).addAgentSysMposActivityRewardInfo(activityRewardMap);
	        ...
	        //删除缓存
	        redisUtils.remove(RedisNameConstants.zfpay_sys_activity_reward_list_02+params.get("activity_id").toString());//活动奖励列表
	 ...
	 */
	@Transactional
	public R addAgentSysMposActivityRewardInfo(Map<String, Object> activityRewardMap) {
		try {
			int i = agentSysMposActivityRewardInfoMapper.addAgentSysMposActivityRewardInfo(activityRewardMap);
			if(i != 1) {
	...
	public Map<String, Object> getAgentSysMposActivityRewardInfoById(String id) {
		return agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoById(id);
	}


	/**
	 * 修改线上活动(MPOS)奖励类型（只能编辑待发布、未删除的状态）
	 */
	@Override
	public R editAgentSysMposActivityRewardInfo(Map<String, Object> params) {
		try {
			if(!ShiroUtils.getSysUser().isAuth()) {
				return R.error(Type.WARN, "身份信息未认证，不能操作");
			}
			...
			int i=0;
			//（1）查询old参数信息
			Map<String, Object> oldValue = agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoById(params.get("reward_id").toString());
			//（2）更新
			...
			i = agentSysMposActivityRewardInfoMapper.updateAgentSysMposActivityRewardInfo(params);
			if(i != 1) {
				return R.error(Type.WARN, "更新失败");
			}
			//（3）查询new参数信息
			Map<String, Object> newValue = agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoById(params.get("reward_id").toString());
			//（4）记录修改记录
			params.put("table_name", SysTableNameConstant.t_sys_mpos_activity_reward_info);
			...
			//（5）删除缓存
	        redisUtils.remove(RedisNameConstants.zfpay_sys_activity_reward_list_02+newValue.get("activity_id").toString());//活动奖励列表
	        redisUtils.remove(RedisNameConstants.zfpay_sys_activity_reward_02+params.get("reward_id").toString());//活动奖励详情
			return R.ok("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
			return R.error(Type.ERROR, "操作异常");
	...
	/**
	 * 批量删除线上活动(MPOS)奖励类型(只能待发布、未删除的状态才能)
	 */
	@Override
	public R batchRemoveAgentSysMposActivityRewardInfo(Map<String, Object> params) {
		if(!ShiroUtils.getSysUser().isAuth()) {
			return R.error(Type.WARN, "身份信息未认证，不能操作");
		}
		if(StringUtils.isEmpty(StringUtil.getMapValue(params, "remark"))) {
        ...
        for(int i=0;i<reward_ids.length;i++) {
        	...
        	//依次删除每一个公益(USDT)
        	R result = SpringUtils.getAopProxy(this).removeAgentSysMposActivityRewardInfo(activityRewardMap);
        	if(R.Type.SUCCESS.value.equals(result.get("code").toString())) {
            	success_num++;
            }else {
            	failure_num++;
	...
	/**
	 * 删除单个线上活动(MPOS)奖励类型(只能待发布、未删除的状态才能)
	 * @param activityRewardMap
	 * @return
	 */
	@Transactional
	public R removeAgentSysMposActivityRewardInfo(Map<String, Object> activityRewardMap) {
		try {
			int i=0;
			//（1）查询old参数信息
			Map<String, Object> oldValue = agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoById(activityRewardMap.get("reward_id").toString());
			//（2）更新状态
			...
			System.out.println(activityRewardMap);
			i = agentSysMposActivityRewardInfoMapper.updateAgentSysMposActivityRewardInfoDel(activityRewardMap);
			if(i != 1) {
				return R.error(Type.WARN, "活动(MPOS)奖励编号"+activityRewardMap.get("reward_id").toString()+"：删除失败，只能删除待发布未删除的活动");
			}
			//（3）查询new参数信息
			Map<String, Object> newValue = agentSysMposActivityRewardInfoMapper.getAgentSysMposActivityRewardInfoById(activityRewardMap.get("reward_id").toString());
			//（4）记录修改记录
			activityRewardMap.put("table_name", SysTableNameConstant.t_sys_mpos_activity_reward_info);
			...
			//（5）删除缓存
	        redisUtils.remove(RedisNameConstants.zfpay_sys_activity_reward_list_02+newValue.get("activity_id").toString());//活动奖励列表
	        redisUtils.remove(RedisNameConstants.zfpay_sys_activity_reward_02+activityRewardMap.get("reward_id").toString());//活动奖励详情
			return R.ok("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			return R.error(Type.WARN, "活动(MPOS)奖励编号"+activityRewardMap.get("reward_id").toString()+"：删除异常");
		}
	}
