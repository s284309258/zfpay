...
import com.ruoyi.project.deveagent.usermpos.mapper.AgentUserMposUnbindRecordInfoMapper;
 ...
 */
@Service
public class AgentUserMposUnbindRecordInfoServiceImpl implements AgentUserMposUnbindRecordInfoService {
	...
	@Autowired
	private AgentUserMposUnbindRecordInfoMapper agentUserMposUnbindRecordInfoMapper;
	@Autowired
	 ...
	 */
	@Override
	public List<Map<String, Object>> getAgentUserMposUnbindRecordInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserMposUnbindRecordInfoMapper.getAgentUserMposUnbindRecordInfoList(params);
	}
	 ...
	 */
	@Override
	public List<AgentUserMposUnbindRecordInfo> selectAgentUserMposUnbindRecordInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserMposUnbindRecordInfoMapper.selectAgentUserMposUnbindRecordInfoList(params);
	}
	 ...
	 */
	@Override
	public R batchAgentSysAuditUserMposUnbindRecordInfo(Map<String, Object> params) {
        ...
        if(TypeStatusConstant.user_trapos_unbind_record_info_status_09.equals(StringUtil.getMapValue(params, "status"))) {
        	//批量审核通过
        	return this.batchAcceptAgentUserMposUnbindRecordInfo(params);
        }else if(TypeStatusConstant.user_trapos_unbind_record_info_status_08.equals(StringUtil.getMapValue(params, "status"))){
        	//批量审核不通过
        	return this.batchRefuseAgentUserMposUnbindRecordInfo(params);
        }else {
	 ...
	 * @return
	 */
	private R batchAcceptAgentUserMposUnbindRecordInfo(Map<String, Object> params) {
        ...
        for(int i=0;i<unbind_ids.length;i++) {
        	...
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).acceptAgentUserMposUnbindRecordInfo(unbindMap);
	 ...
	 */
	@Transactional
	public R acceptAgentUserMposUnbindRecordInfo(Map<String, Object> unbindMap) {
		try {
			int i=0;
			//（1）更新审核状态
			...
			i = agentUserMposUnbindRecordInfoMapper.updateAgentUserMposUnbindRecordInfoStatus(unbindMap);
			if(i!=1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				return R.error(Type.WARN, "编号"+unbindMap.get("unbind_id").toString()+"：解绑状态更新失败");
			}
			//（2）根据id查询详情，主要是获取设备号（机器编号）
			Map<String, Object> unbindDetailMap = agentUserMposUnbindRecordInfoMapper.getAgentUserMposUnbindRecordInfoById(unbindMap.get("unbind_id").toString());
			//（3）更新该POS机的分配状态
			...
			i = agentSysMposInfoMapper.updateAgentSysMposInfoDisStatus(unbindMap);
			if(i!=1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			...
			i = agentUserMposInfoMapper.updateAgentSameChainUserMposInfoDel(unbindMap);
			if(i<1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
	 ...
	 * @return
	 */
	private R batchRefuseAgentUserMposUnbindRecordInfo(Map<String, Object> params) {
		int success_num = 0;//成功数量
        int failure_num = 0;//失败数量
        String failure_msg = "";//失败消息
        //拼接id转换成long型数组
        ...
        for(int i=0;i<unbind_ids.length;i++) {
        	...
        	unbindMap.put("unbind_id", unbind_ids[i]);
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).refuseAgentUserMposUnbindRecordInfo(unbindMap);
        	if(R.Type.SUCCESS.value.equals(result.get("code").toString())) {
            	success_num++;
            }else {
            	failure_num++;
	 ...
	 */
	@Transactional
	public R refuseAgentUserMposUnbindRecordInfo(Map<String, Object> unbindMap) {
		try {
			int i=0;
			//（1）更新审核状态
			unbindMap.put("old_status", TypeStatusConstant.user_trapos_unbind_record_info_status_00);//旧状态：待审核
