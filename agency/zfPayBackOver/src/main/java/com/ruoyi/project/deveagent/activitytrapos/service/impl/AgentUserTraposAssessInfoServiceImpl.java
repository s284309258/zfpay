...
import com.ruoyi.project.deveagent.activitytrapos.mapper.AgentUserTraposAssessInfoMapper;
 ...
 */
@Service
public class AgentUserTraposAssessInfoServiceImpl implements AgentUserTraposAssessInfoService {
	 ...
	 @Autowired
	 private AgentUserTraposAssessInfoMapper agentUserTraposAssessInfoMapper;
	 @Autowired
	 ...
	 */
	@Override
	public List<Map<String, Object>> getAgentUserTraposAssessInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		params.put("today_date", TimeUtil.getDayString());
		return agentUserTraposAssessInfoMapper.getAgentUserTraposAssessInfoList(params);
	}
	 ...
	 */
	@Override
	public List<AgentUserTraposAssessInfo> selectAgentUserTraposAssessInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		params.put("today_date", TimeUtil.getDayString());
		return agentUserTraposAssessInfoMapper.selectAgentUserTraposAssessInfoList(params);
	}
	...
	@Override
	@Transactional
	public R addAgentUserTraposAssessInfo(Map<String, Object> params) {
		try {
...
//			}
			...
			int m = agentUserTraditionalPosInfoMapper.getAgentJoinTraditionalposAssesInfoNum(params);
			if(m != sn_lists.length) {
				return R.error(Type.WARN, "只能选择直属的未删除的传统POS");
			}
			//（4）校验该POS是否已经参与扣款活动
			for(int j=0;j<sn_lists.length;j++) {
				//查询是否已经申请过，存在表中
				params.put("sn", sn_lists[j]);
				int n = agentUserTraposAssessInfoMapper.getAgentHaveJoinSnNum(params);
				if(n > 0) {
					return R.error(Type.WARN, "选择的传统POS中有参与扣款项目，不能设置");
				}
			}
			//（5）新增活动信息
			...
			i = agentUserTraposAssessInfoMapper.addAgentUserTraposAssessInfo(params);
			if(i != 1) {
				return R.error(Type.WARN, "传统POS系统考核新增失败");
			}
			//（6）建立任务对象，并保存任务信息
			Job job = new Job();
			job.setJobName("用户传统POS系统考核"+params.get("id").toString());//任务名称
			job.setJobGroup("DEFAULT");//任务组名
			job.setInvokeTarget("activitySettlementTask.traposAssessTaskSettlement('"+params.get("id").toString()+"')");//调用目标字符串
			...
			job.setRemark("用户传统POS系统考核:"+params.get("id").toString());
			...
			i = agentUserTraposAssessInfoMapper.updateAgentUserTraposAssessInfoJobId(params);
			if(i != 1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
	 ...
	 */
	@Override
	public R batchRemoveAgentUserTraposAssessInfo(Map<String, Object> params) {
		if(!ShiroUtils.getSysUser().isAuth()) {
			return R.error(Type.WARN, "身份信息未认证，不能操作");
		}
		if(StringUtils.isEmpty(StringUtil.getMapValue(params, "remark"))) {
        ...
        for(int i=0;i<assess_ids.length;i++) {
        	...
        	R result = SpringUtils.getAopProxy(this).removeAgentUserTraposAssessInfo(assessMap);
	 ...
	 */
	@Transactional
	public R removeAgentUserTraposAssessInfo(Map<String, Object> assessMap) {
		try {
			//（1）根据编号查询考核详情
			Map<String, Object> userTraposAssessMap = agentUserTraposAssessInfoMapper.getAgentUserTraposAssessInfoById(assessMap.get("assess_id").toString());
			int i=0;
			i = agentUserTraposAssessInfoMapper.deleteAgentUserTraposAssessInfo(assessMap);
			if(i != 1) {
				return R.error(Type.WARN, "考核(传统POS)编号"+assessMap.get("assess_id").toString()+"：删除失败");
			}
			//（3）删除调度任务
			R deleteResult = jobServiceImpl.removeJob(Long.parseLong(userTraposAssessMap.get("job_id").toString()));
			...
			e.printStackTrace();
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			return R.error(Type.WARN, "考核(传统POS)编号"+assessMap.get("assess_id").toString()+"：删除异常");
		}
	}
	 ...
	 */
	@Override
	public Map<String, Object> getAgentUserTraposAssessInfoById(String id) {
		return agentUserTraposAssessInfoMapper.getAgentUserTraposAssessInfoById(id);
	}
