 ...
 */
@Service
public class AgentUserMposInfoServiceImpl extends BasicSerivce implements AgentUserMposInfoService {
	...
	@Override
	public List<Map<String, Object>> getAgentUserMposRecallList(Map<String, Object> params) {
		MapChainParams(params);
		return agentUserMposInfoMapper.getAgentUserMposRecallList(params);
	}

	@Override
	public List<Map<String, Object>> getAgentUserMposRecallHandleList(Map<String, Object> params) {
		MapChainParams(params);
		return agentUserMposInfoMapper.getAgentUserMposRecallHandleList(params);
	}

	@Override
	public int recallUserPos(Map<String, Object> params) {
		MapChainParams(params);
		int cnt = agentUserMposInfoMapper.recallUserPos(params);
		return cnt;
	}

	@Override
	public int agreeRecallUserPos(Map<String, Object> params) {
		MapChainParams(params);
		int cnt = agentUserMposInfoMapper.agreeRecallUserPos(params);
		return cnt;
	}

	@Override
	public int refuseRecallUserPos(Map<String, Object> params) {
		MapChainParams(params);
		int cnt = agentUserMposInfoMapper.refuseRecallUserPos(params);
		return cnt;
	}
...
//			sysPosPolicyService.insertPolicySNInfo(String.valueOf(params.get("policy")),params.get("pos_sns").toString(),params.get("user_id").toString(),"MPOS");
	...
	public R addSingleAgentUserMposInfo(AgentUserMposInfo agentUserMposInfo) {
		try {
			//校验结算低价byqh202006
			AgentUserMposInfo allot = new AgentUserMposInfo();
			allot.setSn(agentUserMposInfo.getSn());
			allot.setUser_id(String.valueOf(getShiroUserInfo().getId()));
			Map<String, Object> basePrice = agentUserMposInfoMapper.getAgentUserMposInfo(allot);
			...
			if(Double.parseDouble(agentUserMposInfo.getCard_settle_price())<Double.parseDouble(card_settle_price)){
				return R.error(Type.WARN, "设备号（机器编号）"+agentUserMposInfo.getSn()+"刷卡结算底价不能低于"+card_settle_price);
			}
			if(Double.parseDouble(agentUserMposInfo.getCloud_settle_price())<Double.parseDouble(cloud_settle_price)){
				return R.error(Type.WARN, "设备号（机器编号）"+agentUserMposInfo.getSn()+"云闪付结算底价不能低于"+cloud_settle_price);
			}
			if(Double.parseDouble(agentUserMposInfo.getSingle_profit_rate())>Double.parseDouble(single_profit_rate)){
				return R.error(Type.WARN, "设备号（机器编号）"+agentUserMposInfo.getSn()+"单笔分润比例不能高于"+single_profit_rate);
			}
			if(Double.parseDouble(agentUserMposInfo.getCash_back_rate())>Double.parseDouble(cash_back_rate)){
				return R.error(Type.WARN, "设备号（机器编号）"+agentUserMposInfo.getSn()+"返现比例不能高于"+cash_back_rate);
			}
			//校验结算低价byqh202006
			...
			sysMposMap.put("old_dis_status", TypeStatusConstant.sys_pos_info_dis_status_0);//旧状态：未分配
			sysMposMap.put("new_dis_status", TypeStatusConstant.sys_pos_info_dis_status_1);//旧状态：已分配
			sysMposMap.put("manager_id", getShiroUserInfo().getManager_id());//代理编号
			sysMposMap.put("sn", agentUserMposInfo.getSn());//设备号（机器编号）
			sysMposMap.put("update_by", ShiroUtils.getLoginName());//代理编号
			sysMposMap.put("up_date", cre_date);//更新日期
			sysMposMap.put("up_time", cre_time);//更新时间
...
//			}
			...
			if(userMposMap!=null) {
				//已存在，更新该关系的信息
				i = agentUserMposInfoMapper.updateAgentUserState0BySNUID(agentUserMposInfo.getSn(),String.valueOf(getShiroUserInfo().getId()));
				if(i != 1) {
					TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
					return R.error(Type.WARN, "设备号（机器编号）"+agentUserMposInfo.getSn()+"新增失败：归属关系修改失败");
				}
				agentUserMposInfo.setId(Integer.parseInt(userMposMap.get("id").toString()));
				i = agentUserMposInfoMapper.updateAgentUserMposInfoByDis(agentUserMposInfo);
				if(i != 1) {
