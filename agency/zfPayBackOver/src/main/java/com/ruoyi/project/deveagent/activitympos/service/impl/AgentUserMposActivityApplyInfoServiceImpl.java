...
import com.ruoyi.project.deveagent.activitympos.service.AgentUserMposActivityApplyInfoService;
 ...
 */
@Service
public class AgentUserMposActivityApplyInfoServiceImpl implements AgentUserMposActivityApplyInfoService {
	 ...
	 */
	@Override
	public List<Map<String, Object>> getAgentUserMposActivityApplyInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserMposActivityApplyInfoMapper.getAgentUserMposActivityApplyInfoList(params);
	}
	 ...
	 */
	@Override
	public List<AgentUserMposActivityApplyInfo> selectAgentUserMposActivityApplyInfoList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserMposActivityApplyInfoMapper.selectAgentUserMposActivityApplyInfoList(params);
	}
	 ...
	 */
	@Override
	public Map<String, Object> getAgentUserMposActivityApplyInfoDetailById(String id) {
		return agentUserMposActivityApplyInfoMapper.getAgentUserMposActivityApplyInfoDetailById(id);
	}
	 ...
	 */
	@Override
	public R batchSysAuditUserMposActivityApplyInfo(Map<String, Object> params) {
        ...
        if(TypeStatusConstant.user_pos_activity_info_status_09.equals(StringUtil.getMapValue(params, "status"))) {
        	//批量审核通过
        	return this.batchAcceptAgentUserMposActivityApplyInfo(params);
        }else if(TypeStatusConstant.user_pos_activity_info_status_08.equals(StringUtil.getMapValue(params, "status"))){
        	//批量审核不通过
        	return this.batchRefuseAgentUserMposActivityApplyInfo(params);
        }else {
	 ...
	 * @return
	 */
	private R batchAcceptAgentUserMposActivityApplyInfo(Map<String, Object> params) {
        ...
        for(int i=0;i<apply_ids.length;i++) {
        	...
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).acceptAgentUserMposActivityApplyInfo(activityApplyMap);
	 ...
	 */
	@Transactional
	public R acceptAgentUserMposActivityApplyInfo(Map<String, Object> activityApplyMap) {
		try {
			int i=0;
			//（1）更新审核状态
			...
			i = agentUserMposActivityApplyInfoMapper.updateAgentUserMposActivityApplyInfoStatus(activityApplyMap);
			if(i!=1) {
				return R.error(Type.WARN, "申请活动(MPOS)编号"+activityApplyMap.get("apply_id").toString()+"：状态更新失败");
			}
			return R.ok("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
			return R.error(Type.WARN, "申请活动(MPOS)编号"+activityApplyMap.get("apply_id").toString()+"：审核异常");
		}
	}
	 ...
	 * @return
	 */
	private R batchRefuseAgentUserMposActivityApplyInfo(Map<String, Object> params) {
		int success_num = 0;//成功数量
        int failure_num = 0;//失败数量
        String failure_msg = "";//失败消息
		 //拼接id转换成long型数组
        ...
        for(int i=0;i<apply_ids.length;i++) {
        	...
        	activityApplyMap.put("apply_id", apply_ids[i]);
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).refuseAgentUserMposActivityApplyInfo(activityApplyMap);
        	if(R.Type.SUCCESS.value.equals(result.get("code").toString())) {
            	success_num++;
            }else {
            	failure_num++;
	 ...
	 */
	@Transactional
	public R refuseAgentUserMposActivityApplyInfo(Map<String, Object> activityApplyMap) {
		try {
			int i=0;
			//（1）更新审核状态
			activityApplyMap.put("manager_id", ShiroUtils.getUserId());//代理编号
			...
			//（2）查询详情，获取sn_list
			Map<String, Object> applyDetailMap = agentUserMposActivityApplyInfoMapper.getAgentUserMposActivityApplyInfoMapById(activityApplyMap.get("apply_id").toString());
			//（3）更新用户pos机的参与活动状态
			...
			int n = agentUserMposInfoMapper.updateAgentUserMposInfoActivityStatus(activityApplyMap);
			if(m != n) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				return R.error(Type.WARN, "申请活动(MPOS)编号"+activityApplyMap.get("apply_id").toString()+"：更新数量不对");
			}
			return R.ok("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
