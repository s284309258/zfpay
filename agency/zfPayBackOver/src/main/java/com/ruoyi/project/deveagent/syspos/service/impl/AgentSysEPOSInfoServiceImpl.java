...
import com.ruoyi.project.deveagent.syspos.mapper.AgentSysEPOSInfoMapper;
 ...
 */
@Service
public class AgentSysEPOSInfoServiceImpl extends BasicSerivce implements AgentSysEPOSInfoService {

    @Autowired
    private ManaSysEditMapper manaSysEditMapper;
    @Autowired
    private AgentSysEPOSInfoMapper agentSysEPOSInfoMapper;
    @Autowired
    ...
    public List<AgentSysTraditionalPosInfo> selectAgentSysTraditionalPosInfoList(Map<String, Object> params) {
        ...
        return agentSysEPOSInfoMapper.selectAgentSysTraditionalPosInfoList(params);
    }
    ...
    public List<Map<String, Object>> getSubAgentSysTraditionalPosInfoList(Map<String, Object> params) {
        params.put("manager_id", ShiroUtils.getUserId());
        return agentSysEPOSInfoMapper.getSubAgentSysTraditionalPosInfoList(params);
    }
    ...
    public R importAgentSysTraditionalPosInfoList(List<AgentSysTraditionalPosInfo> agentSysTraditionalPosInfoList, String account_id) {
        if (StringUtil.isNull(agentSysTraditionalPosInfoList) || agentSysTraditionalPosInfoList.size() == 0){
            return R.error(R.Type.WARN, "导入系统传统POS数据不能为空！");
        }
        //校验代理中付账号编号
        ...
        if(userAccountMap==null) {
            return R.error(R.Type.WARN, "该中付账号无效，请重新选择设置");
        }
    ...
    public R importAgentSysTraditionalPosInfo(AgentSysTraditionalPosInfo agentSysTraditionalPosInfo, int i) {
        try {
            ...
            if(sysParamRateList1.size()<=0) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：刷卡费率数值异常");
            }
            //（3）查询云闪付费率参数是否有效
            ...
            if(sysParamRateList2.size()<=0) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：云闪付费率数值异常");
            }
            //（4）查询微信费率参数是否有效
            ...
            if(sysParamRateList3.size()<=0) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：微信费率数值异常");
            }
            //（5）查询支付宝费率参数是否有效
            ...
            if(sysParamRateList4.size()<=0) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：支付宝费率数值异常");
            }
            //（6）校验该POS机是否存在（根据设备号（机器编号）查询）
            Map<String, Object> sysTraditionalPosInfoMap = agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoBySn(agentSysTraditionalPosInfo.getSn());
            if(sysTraditionalPosInfoMap!=null) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：该设备号（机器编号）已存在");
            }
            //（7）保存POS机信息
            ...
            int j = agentSysEPOSInfoMapper.addAgentTraditionalPosInfo(agentSysTraditionalPosInfo);
            if(j != 1) {
                return R.error(R.Type.WARN, "第"+(i+1)+"条数据导入失败：数据信息保存失败");
            }
            return R.ok("操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            return R.error(R.Type.ERROR, "第"+(i+1)+"条数据导入失败：数据导入异常");
        }
    }
    ...
    public R addAgentSysTraditionalPosInfo(Map<String, Object> params) {
        ...
        if(!"0".equals(StringUtil.getMapValue(params, "cash_back_type")) && !"1".equals(StringUtil.getMapValue(params, "cash_back_type"))) {
            return R.error(R.Type.WARN, "返现获取类型值异常");
        }
        //（3）查询刷卡费率参数是否有效
        ...
        if(sysParamRateList1.size()<=0) {
            return R.error(R.Type.WARN, "刷卡费率数值异常");
        }
        //（4）查询云闪付费率参数是否有效
        ...
        if(sysParamRateList2.size()<=0) {
            return R.error(R.Type.WARN, "云闪付费率数值异常");
        }
        //（5）查询微信费率参数是否有效
        ...
        if(sysParamRateList3.size()<=0) {
            return R.error(R.Type.WARN, "微信费率数值异常");
        }
        //（6）查询支付宝费率参数是否有效
        ...
        if(sysParamRateList4.size()<=0) {
            return R.error(R.Type.WARN, "支付宝费率数值异常");
        }
        ...
        if(TypeStatusConstant.sys_add_type_1.equals(StringUtil.getMapValue(params, "add_type"))) {
            ...
            return R.error(R.Type.WARN, "新增类型异常");
        }
    }
    ...
    public R addSingleAgentSysTraditionalPosInfo(AgentSysTraditionalPosInfo agentSysTraditionalPosInfo) {
        try {
            ...
            Map<String, Object> sysTraditionalPosInfoMap = agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoBySn(agentSysTraditionalPosInfo.getSn());
            if(sysTraditionalPosInfoMap!=null) {
                return R.error(R.Type.WARN, "设备号（机器编号）"+agentSysTraditionalPosInfo.getSn()+"保存失败：该设备号（机器编号）已存在");
            }
            agentSysTraditionalPosInfo.setManager_id(ShiroUtils.getUserId().toString());//代理编号
            agentSysTraditionalPosInfo.setCre_date(TimeUtil.getDayString());//创建日期
            agentSysTraditionalPosInfo.setCre_time(TimeUtil.getTimeString());//创建时间
            ...
            int j = agentSysEPOSInfoMapper.addAgentTraditionalPosInfo(agentSysTraditionalPosInfo);
            if(j != 1) {
                return R.error(R.Type.WARN, "设备号（机器编号）"+agentSysTraditionalPosInfo.getSn()+"保存失败：数据信息保存失败");
            }
            return R.ok("操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return R.error(R.Type.WARN, "设备号（机器编号）"+agentSysTraditionalPosInfo.getSn()+"新增异常");
        }
    }
    ...
    private R autoAddAgentSysTraditionalPosInfo(AgentSysTraditionalPosInfo agentSysTraditionalPosInfo,
        ...
        if(StringUtils.isEmpty(StringUtil.getMapValue(params, "pos_start"))) {
            return R.error(R.Type.WARN, "设备号（机器编号）起始数值异常");
        }
        if(StringUtils.isEmpty(StringUtil.getMapValue(params, "pos_num"))) {
            return R.error(R.Type.WARN, "POS机数量数值异常");
        }
        //POS机设备号（机器编号）起始号
    ...
    public Map<String, Object> getAgentSysTraditionalPosInfoById(String id) {
        return agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoById(id);
    }
    ...
    public R editAgentSysTraditionalPosInfo(Map<String, Object> params) {
        try {
            if(!ShiroUtils.getSysUser().isAuth()) {
                return R.error(R.Type.WARN, "身份信息未认证，不能操作");
            }
            if(StringUtils.isEmpty(StringUtil.getMapValue(params, "remark"))) {
                return R.error(R.Type.WARN, "操作备注不能为空");
            }
            int i=0;
            //（1）校验代理中付账号编号
            params.put("manager_id", ShiroUtils.getUserId());//代理编号
            ...
            if(sysParamRateList11.size()<=0) {
                return R.error(R.Type.WARN, "VIP秒到(商圈)刷卡费率数值异常");
            }
            //（7）查询old参数信息
            Map<String, Object> oldValue = agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoById(params.get("pos_id").toString());
            //（8）更新系统传统POS信息
            params.put("manager_id", ShiroUtils.getUserId());//代理编号
            ...
            i = agentSysEPOSInfoMapper.updateAgentSysTraditionalPosInfo(params);
            if(i != 1) {
                return R.error(R.Type.WARN, "系统传统POS信息更新失败");
            }
            //（9）查询new参数信息
            Map<String, Object> newValue = agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoById(params.get("pos_id").toString());
            //（10）记录修改记录
            ...
            if(i != 1) {
                TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                return R.error(R.Type.WARN, "修改记录记录失败");
            }
            return R.ok("操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return R.error(R.Type.ERROR, "操作异常");
        }
    }
    ...
    public R removeAgentSysTraditionalPosInfo(Map<String, Object> sysTraditionalPosMap) {
        try {
            ...
            int i = agentSysEPOSInfoMapper.deleteAgentSysTraditionalPosInfo(sysTraditionalPosMap);
            if(i != 1) {
                return R.error(R.Type.WARN, "编号"+sysTraditionalPosMap.get("pos_id").toString()+"：删除失败，只能删除待未分配的POS机");
            }
            return R.ok("操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            return R.error(R.Type.WARN, "编号"+sysTraditionalPosMap.get("pos_id").toString()+"：删除异常");
        }
    }
    ...
    public Map<String, Object> getAgentSysTraditionalPosInfoDetailBySn(String id) {
        return agentSysEPOSInfoMapper.getAgentSysTraditionalPosInfoDetailBySn(id);
    }
}
