 ...
 */
@Service
public class ManaSysParamRateServiceImpl extends BasicSerivce implements ManaSysParamRateService {
	 ...
	 */
	@Override
	public List<ManaSysParamRate> getManaParamRateListByType(String type,String pos_type) {
		List<ManaSysParamRate> sysParamRates = manaSysParamRateMapper.getManaParamRateListByType(type);
		if("".equals(pos_type) || null==pos_type){
			return sysParamRates;
		}
		Integer user_id = getShiroUserInfo().getId();
		List<ManaSysParamRate> newRate = new ArrayList<>();
		if("MPos".equals(pos_type)){
			Map<String,Object> maps = manaSysParamRateMapper.getMinRateByMpos(user_id);
			if(maps!=null){
				if("2".equals(type)){
					Double card_settle_price = Double.parseDouble(maps.get("card_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate>=card_settle_price){
							newRate.add(manaSysParamRate);
						}
					}
				}else if("3".equals(type)){
					Double cloud_settle_price = Double.parseDouble(maps.get("cloud_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate>=cloud_settle_price){
							newRate.add(manaSysParamRate);
						}
					}
				}else if("9".equals(type)){
					Double single_profit_rate = Double.parseDouble(maps.get("single_profit_rate").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate<=single_profit_rate){
							newRate.add(manaSysParamRate);
						}
					}
				}else if("10".equals(type)){
					Double cash_back_rate = Double.parseDouble(maps.get("cash_back_rate").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate<=cash_back_rate){
							newRate.add(manaSysParamRate);
						}
					}
				}
			}
		}else if("TraditionalPos".equals(pos_type)){
			Map<String,Object> maps = manaSysParamRateMapper.getMinRateByTraspos(user_id);
			if(maps!=null){
				if("2".equals(type)){
					Double card_settle_price = Double.parseDouble(maps.get("card_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
					...
					Double weixin_settle_price = Double.parseDouble(maps.get("weixin_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate>=weixin_settle_price){
							newRate.add(manaSysParamRate);
						}
					}
				}else if("7".equals(type)){
					Double zhifubao_settle_price = Double.parseDouble(maps.get("zhifubao_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate>=zhifubao_settle_price){
							newRate.add(manaSysParamRate);
						}
					}
				}else if("11".equals(type)){
					Double mer_cap_fee = Double.parseDouble(maps.get("mer_cap_fee").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
						Double rate = Double.parseDouble(manaSysParamRate.getRate());
						if(rate>=mer_cap_fee){
							newRate.add(manaSysParamRate);
		...
		}else if("EPos".equals(pos_type)){
			Map<String,Object> maps = manaSysParamRateMapper.getMinRateByEpos(user_id);
			if(maps!=null){
				if("2".equals(type)){
					Double card_settle_price = Double.parseDouble(maps.get("card_settle_price").toString());
					for(ManaSysParamRate manaSysParamRate : sysParamRates){
