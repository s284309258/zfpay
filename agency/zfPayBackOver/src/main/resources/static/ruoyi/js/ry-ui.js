...
(function ($) {
    $.extend({
    	...
    	table: {
    		...
    		// 导出待处理数据
    		exportDealExcel: function(formId) {
    			$.modal.confirm("确定导出所有待处理的" + $.table._option.modalName + "吗？", function() {
	    			var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
	    			$.modal.loading("正在导出数据，请稍后...");
	    			$.post($.table._option.exportDealUrl, $("#" + currentId).serializeArray(), function(result) {
	    				if (result.code == web_status.SUCCESS) {
	    			        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
	    				} else if (result.code == web_status.WARNING) {
	                        $.modal.alertWarning(result.msg)
            ...
            // 导入POS机数据
            importPosExcel: function(formId) {
            	var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
            	layer.open({
            		type: 1,
            		area: ['400px', '230px'],
            		...
            		btn1: function(index, layero){
            			...
            			if ($("#account_id").val()==''){
            				$.modal.msgWarning("请选择代理中付账号，若无中付账号请去设置。");
            				return false;
            			}
            			var index = layer.load(2, {shade: false});
            			$.modal.disable();
            			...
            			$.ajax({
            				url: $.table._option.importPosUrl,
            				data: formData,
            				cache: false,
            				contentType: false,
            				processData: false,
