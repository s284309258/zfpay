...
import com.ruoyi.project.deveagent.usertrapos.mapper.AgentUserApplyCardrateTraposRecordMapper;
 ...
 */
@Service
public class AgentUserApplyCardrateTraposRecordServiceImpl implements AgentUserApplyCardrateTraposRecordService {
	
	
	@Autowired
	private AgentUserApplyCardrateTraposRecordMapper agentUserApplyCardrateTraposRecordMapper;
	@Autowired
	...
	/**
	 * 查询用户用户费率申请记录(传统POS)列表
	 */
	@Override
	public List<Map<String, Object>> getAgentUserApplyCardrateTraposRecordList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserApplyCardrateTraposRecordMapper.getAgentUserApplyCardrateTraposRecordList(params);
	}
	
	
	/**
	 * 导出用户用户费率申请记录(传统POS)列表
	 */
	@Override
	public List<AgentUserApplyCardrateTraposRecord> selectAgentUserApplyCardrateTraposRecordList(Map<String, Object> params) {
		params.put("manager_id", ShiroUtils.getUserId());
		return agentUserApplyCardrateTraposRecordMapper.selectAgentUserApplyCardrateTraposRecordList(params);
	}
	 ...
	 */
	@Override
	public R batchAgentSysAuditUserApplyCardrateTraposRecord(Map<String, Object> params) {
        ...
        if(TypeStatusConstant.user_apply_cardrate_record_status_09.equals(StringUtil.getMapValue(params, "status"))) {
        	//批量审核通过
        	return this.batchAcceptAgentUserApplyCardrateTraposRecord(params);
        }else if(TypeStatusConstant.user_apply_cardrate_record_status_08.equals(StringUtil.getMapValue(params, "status"))){
        	//批量审核不通过
        	return this.batchRefuseAgentUserApplyCardrateTraposRecord(params);
        }else {
	 ...
	 * @return
	 */
	private R batchAcceptAgentUserApplyCardrateTraposRecord(Map<String, Object> params) {
        ...
        for(int i=0;i<apply_ids.length;i++) {
        	...
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).acceptAgentUserApplyCardrateTraposRecord(applyMap);
	 ...
	 */
	@Transactional
	public R acceptAgentUserApplyCardrateTraposRecord(Map<String, Object> applyMap) {
		try {
			int i=0;
			//（1）更新审核状态
			...
			i = agentUserApplyCardrateTraposRecordMapper.updateAgentUserApplyCardrateTraposRecordStatus(applyMap);
			if(i!=1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				return R.error(Type.WARN, "申请(MPOS)编号"+applyMap.get("apply_id").toString()+"：状态更新失败");
			}
			//（2）根据id查询详情
			Map<String, Object> applyDetailMap = agentUserApplyCardrateTraposRecordMapper.getAgentUserApplyCardrateTraposRecordById(applyMap.get("apply_id").toString());
			//（3）更新系统MPOS机的刷卡费率
			...
			i = agentSysTraditionalPosInfoMapper.updateSysTraditionalPosInfoCreditCardRate(applyMap);
			if(i!=1) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
	 ...
	 * @return
	 */
	private R batchRefuseAgentUserApplyCardrateTraposRecord(Map<String, Object> params) {
		int success_num = 0;//成功数量
        int failure_num = 0;//失败数量
        String failure_msg = "";//失败消息
		 //拼接id转换成long型数组
        ...
        for(int i=0;i<apply_ids.length;i++) {
        	...
        	applyMap.put("apply_id", apply_ids[i]);
        	//依次冻结每一个账号
        	R result = SpringUtils.getAopProxy(this).refuseAgentUserApplyCardrateTraposRecord(applyMap);
        	if(R.Type.SUCCESS.value.equals(result.get("code").toString())) {
            	success_num++;
            }else {
            	failure_num++;
	 ...
	 */
	@Transactional
	public R refuseAgentUserApplyCardrateTraposRecord(Map<String, Object> applyMap) {
		try {
			int i=0;
			//（1）更新审核状态
			applyMap.put("manager_id", ShiroUtils.getUserId());//代理编号
