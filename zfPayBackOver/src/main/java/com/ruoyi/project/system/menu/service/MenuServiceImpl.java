...
public class MenuServiceImpl implements IMenuService
{
    ...
    public List<Ztree> initZtree(List<Menu> menuList, List<String> roleMenuList, boolean permsFlag)
    {
        ...
        for (Menu menu : menuList)
        {
            ...
            ztree.setId(menu.getMenuId());//节点id=菜单id
            ztree.setpId(menu.getParentId());//节点父id=父菜单id
            ztree.setName(transMenuName(menu, permsFlag));
            ztree.setTitle(menu.getMenuName());//节点标题=菜单名称
            //如果角色已存在菜单列表不为空，若菜单列表包含菜单id和权限字符串，则选中
            if (isCheck)
            {
    ...
    public String transMenuName(Menu menu, boolean permsFlag)
    {
        ...
        sb.append(menu.getMenuName());//菜单名称
        //如果需要显示权限标识
        if (permsFlag)
        {
        	//追加字符
